...
- Separate workers live in `workers/turn-engine/`, `workers/whisper-worker/`, and `workers/combat-engine/`. Each must be deployed from its own directory.
- `workers/combat-engine/` was brought onto main on 2026-06-10 (cherry-picked from `claude/combat-engine-impl`, where it was originally developed). The deployed `shellforge-combat-engine` worker predates this — after any combat change, redeploy from `workers/combat-engine/` per the deploy rules below.
- Combat DB migrations live in `combat/migration.sql` + `combat/migration_accept_flow.sql`; seed data in `combat/seed.sql` (generated by `combat/build-seed.js`).
